// ReputationFlow - Trustpilot Content Script
(function () {
  'use strict';
  ...
  function injectButtons() {
    const reviewElements = document.querySelectorAll('[class*="review-card"], article[class*="review"]');
  ...
  function extractReviewText(reviewEl) {
    const textEl = reviewEl.querySelector('[class*="review-content"], p[class*="text"], [data-service-review-text-typography]');
    return textEl ? textEl.textContent.trim() : '';
  }

  function extractRating(reviewEl) {
    const starEl = reviewEl.querySelector('[class*="star-rating"], img[alt*="Rated"]');
    if (starEl) {
      const alt = starEl.getAttribute('alt') || '';
  ...
  function getBusinessName() {
    const titleEl = document.querySelector('h1[class*="title"], span[class*="multi-size-header"]');
